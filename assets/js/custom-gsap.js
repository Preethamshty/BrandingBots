...
portfolioMatch.add("(max-width: 767px)", () => {
    // Slightly earlier start/end on mobile keeps card transitions crisp on touch scroll.
    initPortfolioPanels('top 14%', "bottom 92%", 0.85);
});

portfolioMatch.add("(min-width: 768px)", () => {
    initPortfolioPanels('top 20%', "bottom 100%", 1);
});

// If a portfolio panel uses a background <video>, refresh ScrollTrigger after it can report dimensions.
// This prevents small "jumps" when scrolling between panels while the video is initializing.
document.querySelectorAll(".portfolio-bg-video").forEach((video) => {
